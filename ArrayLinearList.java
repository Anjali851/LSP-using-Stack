...
/**
 * Write a description of class ArrayLinearList here.
 *
 * @author (your name)
...
{
    //Instance variables
    int n;//acts as pointer.Points to last element in the array
    int arr[];
    ...
    {
        this.n = 0;
        this.arr = new int[100];//setting size of array 100
    }
    ...
    public ArrayLinearList(int n,int[] ar)
    {
        ...
        for(int i=0;i<n;i++)
        {
            this.arr[i] = ar[i];//copying the given array elements to the current array
        }
    }
    ...
    public int get(int index)
    {
        if(index >= n || index < 0)
        {
            System.out.println("\nIndex is out of bound.-999 will be returned");
            return -999;
        }
        else
        {
